#!/bin/bash
#
# Policy-aware GPU submitter for SolidNES DeepSolid smoke jobs.

set -eo pipefail

SOLIDNES_SUBMIT_SCRIPT="${SOLIDNES_SUBMIT_SCRIPT:-scripts/slurm/run_deepsolid_gpu_smoke.slurm}"
SOLIDNES_SLURM_ACCOUNT="${SOLIDNES_SLURM_ACCOUNT:-hmt03}"
SOLIDNES_JOB_NAME="${SOLIDNES_JOB_NAME:-solidnes-gpu-smoke}"
...
SOLIDNES_GPU_LEGACY_MIN_GPUS="${SOLIDNES_GPU_MIN_GPUS:-}"
SOLIDNES_GPU_LEGACY_MIN_CPUS="${SOLIDNES_GPU_MIN_CPUS:-}"
SOLIDNES_GPU_TARGET_GPUS="${SOLIDNES_GPU_TARGET_GPUS:-${SOLIDNES_GPU_LEGACY_MIN_GPUS:-0}}"
...
SOLIDNES_GPU_QUEUE_MIN_GPUS_DEFAULT="${SOLIDNES_GPU_TARGET_GPUS}"
if [ -z "${SOLIDNES_GPU_QUEUE_MIN_GPUS_DEFAULT}" ] || [ "${SOLIDNES_GPU_QUEUE_MIN_GPUS_DEFAULT}" = "0" ]; then
  SOLIDNES_GPU_QUEUE_MIN_GPUS_DEFAULT="4"
fi
SOLIDNES_GPU_QUEUE_MIN_GPUS="${SOLIDNES_GPU_QUEUE_MIN_GPUS:-$SOLIDNES_GPU_QUEUE_MIN_GPUS_DEFAULT}"
SOLIDNES_GPU_QUEUE_MIN_CPUS="${SOLIDNES_GPU_QUEUE_MIN_CPUS:-${SOLIDNES_GPU_LEGACY_MIN_CPUS:-64}}"
SOLIDNES_GPU_QUEUE_MEMORY_MB="${SOLIDNES_GPU_QUEUE_MEMORY_MB:-64000}"
SOLIDNES_GPU_QUEUE_MODE="${SOLIDNES_GPU_QUEUE_MODE:-auto}"
SOLIDNES_GPU_PRECISION_PROFILE="${SOLIDNES_GPU_PRECISION_PROFILE:-fp64}"
...
export SOLIDNES_CONDA_ENV="${SOLIDNES_CONDA_ENV:-solidnes-deepsolid-jax0430-cuda12-probe}"
...
echo "SolidNES GPU submission policy:"
