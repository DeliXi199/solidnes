#!/usr/bin/env python
"""Build-only gradient-step check for FermiNet PBC penalty states."""
...
def main() -> int:
    ...
    updated_value = ferminet_pbc_penalty_objective(
        adapter,
        updated_params,
        samples,
        penalty_alpha=args.penalty_alpha,
    ...
    _assert_positive_finite(adapter, "grad_norm", grad_norm)
    _assert_positive_finite(adapter, "delta_norm", delta_norm)
    _assert_finite(adapter, "penalty_objective", value)
    _assert_finite(adapter, "updated_penalty_objective", updated_value)

    print("ferminet_pbc_penalty_grad_step: ok")
    ...
    print(f"penalty_objective: {float(value):.12g}")
    print(f"updated_penalty_objective: {float(updated_value):.12g}")
    print(f"grad_l2_norm: {float(grad_norm):.12g}")
    print(f"param_delta_l2_norm: {float(delta_norm):.12g}")
    return 0
